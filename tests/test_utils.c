...
static int test_sort_nodes(void) {
    ...
    /* After sorting, nodes should be in descending order by count */
    TEST_ASSERT(nodes[0]->count >= nodes[4]->count, "sorted descending");
...
static int test_utils_null(void) {
    ...
    sort_nodes(NULL, 0);  /* Should not crash */
